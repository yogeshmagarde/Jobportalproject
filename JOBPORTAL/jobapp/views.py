...
def job(request):
    if(request.session.has_key('sessemail')):
        ...
        return render(request,"jobapp/job.html",{"d":"data inser sucsessfully","res":data1})
    else:
...
def Candidate(request):
    if(request.session.has_key('sessemail')):
        ...
        if request.method=="POST":
            ...
            c.save()
            return render(request,"jobapp/Candidate.html",{"res1":data1,"msg":"Apply sucsessfully"})
        return render(request,"jobapp/Candidate.html",{"res1":data1,"jid":(request.GET["q"])})
    else:
        return redirect('login')
...
def resume(request):
    ...
    if request.method=="POST":
        ...
        r1.save()
        return render(request,"jobapp/show.html",{"y":year,"b":branch,"c":course,"res":data,"d":Resume.objects.all()})
    return render(request,"jobapp/resume.html",{"y":year,"b":branch,"c1":course,"res":data,"d":Resume.objects.all()})
def show(request):
    return render(request,"jobapp/show.html")
